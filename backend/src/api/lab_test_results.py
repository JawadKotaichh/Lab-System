...
from src.schemas.Lab_Test_Result import Lab_test_result, update_lab_test_result_model
...
@router.get("/", response_model=List[DBLab_test_result])
async def get_all_lab_test_results():
    return await DBLab_test_result.find_all().to_list()
...
async def delete_patient(lab_test_result_id: str):
    if not ObjectId.is_valid(lab_test_result_id):
        raise HTTPException(400, "Invalid lab_test_result ID")
