...
async def get_completed_and_total_results( visit_id: str):
    all_items = DBLab_test_result.find(DBLab_test_result.visit_id == PydanticObjectId(visit_id))
    total = 0.0
    completed = 0
    totalPrice = 0.0
    ...
    patient = await DBPatient.find_one(DBPatient.id==PydanticObjectId(visit.patient_id))
    if not patient:
        raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
    ...
    async for item in all_items:
        total+=1
        lab_test = await DBLab_test_type.find_one(DBLab_test_type.id==PydanticObjectId(item.lab_test_type_id))
...
async def get_Lab_panel_with_page_size(page_number:int,page_size:int,visit_id:str|None=Query(None),result:str|None=Query(None),lab_test_type_id:str|None=Query(None)):
