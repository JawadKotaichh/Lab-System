...
async def update_Insurance_company(
    insurance_company_id: str, update_data: update_insurance_company
):
    if not PydanticObjectId.is_valid(insurance_company_id):
        raise HTTPException(400, "Invalid insurance_company ID")

    existing_insurance_company = await DBInsurance_company.find_one(
    ...
    if update_data.insurance_company_name is not None:
        existing_insurance_company.insurance_company_name = (
            update_data.insurance_company_name
        )
