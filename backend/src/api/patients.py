...
async def get_patients_with_page_size(
    ...
    async for patient in cursor:
        insuranceCompany = await DBInsurance_company.find_one(DBInsurance_company.id==PydanticObjectId(patient.insurance_company_id))
        insurance_name = (
            insuranceCompany.insurance_company_name
            if insuranceCompany else
            "Unknown"
        )
        patients.append({
...
async def create_patient(data: Patient):
    ...
    db_patient = DBPatient(
        name=data.name, gender=data.gender, DOB=data.DOB, phone_number=data.phone_number,insurance_company_id=PydanticObjectId(data.insurance_company_id)
    )
...
async def update_patient(patient_id: str, update_data: update_patient_model):
    ...
    existing_patient = await DBPatient.find_one(DBPatient.id == PydanticObjectId(patient_id))
