...
const EditVisitPage: React.FC = () =>{
    ...
    const handleSaveAll = async() => {
         try{
            ...
            item => 
            {   
                const url = `/patients/${patient_id}/visits/${visit_id}/lab_tests_results/${item.lab_test_result_id}`;
    ...
    setLoading(true);
    fetchLabTestResults(patient_id, visit_id)
        .then((data) => {
        setResults(data);
        setLoading(false);
        })
    ...
    <div className="p-8 bg-white">
        <h1 className="text-2xl font-semibold mb-6">Edit Lab Results</h1>
        {results.length === 0 ? (
